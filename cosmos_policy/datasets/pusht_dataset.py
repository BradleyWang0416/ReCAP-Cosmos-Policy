...
class PushTDataset(Dataset):
    def __init__(
        ...
        hdf5_files = get_hdf5_files(data_dir, task_split = self.task_split)
        ...
        if self.demonstration_sampling_prob > 0:  # Only load demos if they are used
            # Collect all (file, demo_key) pairs first
            all_episode_refs = []
            ...
            # Randomly sample N episodes if max_num_episodes is set
            if max_num_episodes > 0 and max_num_episodes < len(all_episode_refs):
                all_episode_refs = random.sample(all_episode_refs, max_num_episodes)
