...
"""
Precomputes Reason1 text embeddings for PushT task descriptions and saves them to disk.
Required for training with the T2V (Cosmos Predict2.5) backbone.

Usage:
    uv run -m cosmos_policy.datasets.save_pusht_reason1_embeddings --data_dir <DATA_DIR>

Example:
    uv run -m cosmos_policy.datasets.save_pusht_reason1_embeddings \\
        --data_dir /data/PushT-Cosmos-Policy/success_only
...
def parse_args():
    parser = argparse.ArgumentParser(description="Precompute Reason1 text embeddings for PushT task descriptions")
    parser.add_argument(
        "--data_dir",
        type=str,
        required=True,
        help="Directory containing PushT dataset (same dir as t5_embeddings.pkl)",
    )
    parser.add_argument(
        "--ckpt_path",
        type=str,
...
def main():
    ...
    print("Loading PushT dataset to collect unique task descriptions...")
    dataset = PushTDataset(data_dir=args.data_dir)
