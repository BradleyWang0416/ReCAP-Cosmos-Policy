...
def get_latent_indices_from_model_config(model):
    ...
    Examples:
        ...
        Real-robot bg2 (state_t=6, min_conditional_frames=3):
            Sequence: [blank, curr_proprio, curr_primary_img, action, future_proprio, future_primary_img]
            Returns: (1, 2, 4, 5)
    """
    ...
    if state_t == 9 and min_conditional_frames == 4:
        ...
        # PushT setup: [blank, proprio, primary, action, future_proprio, future_primary, value]
        return (1, 2, 4, 5)
    elif state_t == 9 and min_conditional_frames == 3:
        # PushT Ret: [blank, proprio, primary, action, future_proprio, future_primary, value, ret0, ret1]
        return (1, 2, 4, 5)
    elif state_t == 6 and min_conditional_frames == 3:
        # Real-robot bg2: [blank, proprio, primary, action, future_proprio, future_primary]
        # Same non-value layout as PushT — value slot is absent.
        return (1, 2, 4, 5)
    else:
        raise ValueError(
...
def extract_action_chunk_from_latent_sequence(
    output_latent: torch.Tensor, action_shape: tuple, action_indices: torch.Tensor, decoder=None
) -> torch.Tensor:
    """
    ...
    Args:
        ...
        decoder: Optional LatentExtraction module. If provided, uses MLP decoder instead of tile-averaging.
    ...
    if decoder is not None:
        # MLP decoder path
        from cosmos_policy.models.policy_text2world_model import extract_action_chunk_from_latent
        return extract_action_chunk_from_latent(action_latent_frame, action_shape, decoder=decoder)
...
def get_action(
    ...
    with torch.inference_mode():
        ...
        if cfg.suite == "libero":
            ...
            WRIST_IMAGE2_IDX = 1
            IMAGE_IDX = 2
        elif cfg.suite == "robotwin" or cfg.suite == "robotwin_ret":
            # Same slot layout as ALOHA: left_camera + right_camera as wrists,
            # head_camera as primary. robotwin_ret reuses the same camera set;
            # the RAG-specific slot ordering is handled in the image_sequence
            # builder below (3-camera retrieval branch).
            all_camera_images = [
                obs["left_wrist_image"],
                obs["right_wrist_image"],
                obs["primary_image"],
            ...
            WRIST_IMAGE2_IDX = 1
            IMAGE_IDX = 2
        elif cfg.suite == "pusht" or cfg.suite == "pusht_ret":
            # PushT: no wrist image, single primary (third-person) image
            all_camera_images = [obs["primary_image"]]
            IMAGE_IDX = 0
        elif cfg.suite == "real_robot":
            # Real-robot bg2 bimanual: single third-eye camera, no wrist images.
            # Same layout as PushT (primary only); pairs with state_t=6 model.
            all_camera_images = [obs["primary_image"]]
            IMAGE_IDX = 0
        else:
        ...
        # Retrieval-specific state
        retrieved_video_start_latent_idx = -1
        retrieved_video_end_latent_idx = -1
        retrieved_action_latent_idx = -1
        retrieved_state_latent_idx = -1
        retrieved_actions_tensor = None
        retrieved_proprio_tensor = None

        primary_image_duplicated = duplicate_array(
            primary_image, total_num_copies=COSMOS_TEMPORAL_COMPRESSION_FACTOR
        )

        if "retrieved_head" in obs:
            # ── 3-camera RoboTwin RAG layout (matches RoboTwinRetDataset.__getitem__):
            #   0: blank (already appended above)
            #   1: ret proprio          (placeholder, value injected via retrieved_proprio_tensor)
            #   2: ret L wrist          (1 latent, stride-sampled distinct frames)
            #   3: ret R wrist
            #   4: ret head
            #   5: ret action           (placeholder, value injected via retrieved_actions_tensor)
            #   6: cur proprio          (placeholder)
            #   7: cur L wrist          (4 dup)
            #   8: cur R wrist
            #   9: cur head
            #  10: action               (placeholder; model fills)
            #  11: future proprio       (placeholder)
            #  12: future L wrist       (4 dup; model fills)
            #  13: future R wrist
            #  14: future head
            _use_ret_image = getattr(cfg, "use_ret_image", True)
            _use_ret_state = getattr(cfg, "use_ret_state", True)

            # Local helper — same preprocessing the pusht_ret branch applies.
            def _prep_ret_frames(arr):
                arr = resize_images(arr, COSMOS_IMAGE_SIZE)
                if cfg.use_jpeg_compression:
                    arr = apply_jpeg_compression_np(arr, quality=95)
                if cfg.trained_with_image_aug:
                    arr = apply_image_transforms(arr)
                return arr

            # Slot 1: ret proprio placeholder
            image_sequence.append(blank_image_duplicated.copy())
            retrieved_state_latent_idx = current_sequence_idx
            current_sequence_idx += 1
            if _use_ret_image and _use_ret_state and "retrieved_proprio" in obs and dataset_stats is not None:
                raw_prop = obs["retrieved_proprio"]
                p_min = dataset_stats["proprio_min"]
                p_max = dataset_stats["proprio_max"]
                norm_prop = 2.0 * ((raw_prop - p_min) / (p_max - p_min + 1e-8)) - 1.0
                retrieved_proprio_tensor = (
                    torch.from_numpy(norm_prop.astype(np.float32))
                    .reshape(1, -1)
                    .repeat(batch_size, 1)
            ...
            # Slots 2-4: ret L wrist / ret R wrist / ret head
            if _use_ret_image:
                ret_head = _prep_ret_frames(obs["retrieved_head"])
                ret_left = _prep_ret_frames(obs["retrieved_left_wrist"])
                ret_right = _prep_ret_frames(obs["retrieved_right_wrist"])
                image_sequence.append(ret_left)
                retrieved_video_start_latent_idx = current_sequence_idx
                current_sequence_idx += len(ret_left) // COSMOS_TEMPORAL_COMPRESSION_FACTOR
                image_sequence.append(ret_right)
                current_sequence_idx += len(ret_right) // COSMOS_TEMPORAL_COMPRESSION_FACTOR
                image_sequence.append(ret_head)
                current_sequence_idx += len(ret_head) // COSMOS_TEMPORAL_COMPRESSION_FACTOR
                retrieved_video_end_latent_idx = current_sequence_idx
            else:
                # When ret images are ablated out the model still expects 3 ret-image
                # slots (training schema is fixed) — fill with zeros so segment ids
                # stay aligned. Each ret camera = COSMOS_TEMPORAL_COMPRESSION_FACTOR frames.
                zeros = np.zeros(
                    (COSMOS_TEMPORAL_COMPRESSION_FACTOR, COSMOS_IMAGE_SIZE, COSMOS_IMAGE_SIZE, 3),
                    dtype=np.uint8,
                )
                image_sequence.append(zeros.copy())
                retrieved_video_start_latent_idx = current_sequence_idx
                current_sequence_idx += 1
                image_sequence.append(zeros.copy())
                current_sequence_idx += 1
            ...
            # Slot 5: ret action placeholder
            image_sequence.append(blank_image_duplicated.copy())
            retrieved_action_latent_idx = current_sequence_idx
            current_sequence_idx += 1
            if "retrieved_actions" in obs and dataset_stats is not None:
                raw_acts = obs["retrieved_actions"]
                a_min = dataset_stats["actions_min"]
                a_max = dataset_stats["actions_max"]
                norm_acts = 2.0 * ((raw_acts - a_min) / (a_max - a_min + 1e-8)) - 1.0
                retrieved_actions_tensor = (
                    torch.from_numpy(norm_acts.astype(np.float32))
                    .unsqueeze(0)
            ...
            # Slot 6: cur proprio placeholder
            image_sequence.append(blank_image_duplicated.copy())
            current_proprio_latent_idx = current_sequence_idx
            current_sequence_idx += 1

            # Slots 7-9: cur L wrist / R wrist / head
            cur_left = duplicate_array(
                all_camera_images[WRIST_IMAGE_IDX], total_num_copies=COSMOS_TEMPORAL_COMPRESSION_FACTOR
            )
            image_sequence.append(cur_left)
            current_wrist_image_latent_idx = current_sequence_idx
            current_sequence_idx += 1
            cur_right = duplicate_array(
                all_camera_images[WRIST_IMAGE2_IDX], total_num_copies=COSMOS_TEMPORAL_COMPRESSION_FACTOR
            )
            image_sequence.append(cur_right)
            ...
            # Slot 10: action placeholder
            image_sequence.append(blank_image_duplicated.copy())
            action_latent_idx = current_sequence_idx
            current_sequence_idx += 1

            # Slot 11: future proprio placeholder
            image_sequence.append(blank_image_duplicated.copy())
            future_proprio_latent_idx = current_sequence_idx
            current_sequence_idx += 1

            # Slots 12-14: future L wrist / R wrist / head (model fills)
            image_sequence.append(cur_left.copy())
            future_wrist_image_latent_idx = current_sequence_idx
            current_sequence_idx += 1
            image_sequence.append(cur_right.copy())
        ...
        elif "retrieved_frames" in obs:
            # ── Retrieval layout: blank | [ret_frame] | [ret_state] | ret_action | cur_frame | cur_state | pred_action | [pred_frame | pred_state]
            # Ablation flags (default True = include slot)
            _use_ret_image = getattr(cfg, 'use_ret_image', True)
            _use_ret_state = getattr(cfg, 'use_ret_state', True)

            # Retrieved frames (skip entirely when use_ret_image=False)
            if _use_ret_image:
                ret_frames = obs["retrieved_frames"]  # (n_ret_frames, H, W, 3) uint8
                ret_frames = resize_images(ret_frames, COSMOS_IMAGE_SIZE)
                if cfg.use_jpeg_compression:
                    ret_frames = apply_jpeg_compression_np(ret_frames, quality=95)
                if cfg.trained_with_image_aug:
                    ret_frames = apply_image_transforms(ret_frames)
                image_sequence.append(ret_frames)
                retrieved_video_start_latent_idx = current_sequence_idx
                num_ret_latents = len(ret_frames) // COSMOS_TEMPORAL_COMPRESSION_FACTOR
                current_sequence_idx += num_ret_latents
                retrieved_video_end_latent_idx = current_sequence_idx


            # Retrieved state slot (skip when use_ret_image=False or use_ret_state=False)
            if _use_ret_image:
                image_sequence.append(blank_image_duplicated.copy())
                retrieved_state_latent_idx = current_sequence_idx
                current_sequence_idx += 1

            # Normalize retrieved proprio (state) — only when state slot exists
            if _use_ret_image and _use_ret_state and "retrieved_proprio" in obs and dataset_stats is not None:
                raw_prop = obs["retrieved_proprio"]  # (proprio_dim,) float32
                p_min = dataset_stats["proprio_min"]
                p_max = dataset_stats["proprio_max"]
                norm_prop = 2.0 * ((raw_prop - p_min) / (p_max - p_min + 1e-8)) - 1.0
                retrieved_proprio_tensor = (
            ...
            # Retrieved action slot
            image_sequence.append(blank_image_duplicated.copy())
            retrieved_action_latent_idx = current_sequence_idx
            current_sequence_idx += 1
            # Normalize retrieved actions
            if "retrieved_actions" in obs and dataset_stats is not None:
                raw_acts = obs["retrieved_actions"]
                a_min = dataset_stats["actions_min"]
                a_max = dataset_stats["actions_max"]
            ...
            # Current state (proprio)
            if cfg.use_proprio:
                image_sequence.append(blank_image_duplicated.copy())
                current_proprio_latent_idx = current_sequence_idx
                current_sequence_idx += 1
    
            # Predicted action slot
            image_sequence.append(blank_image_duplicated.copy())
            action_latent_idx = current_sequence_idx
            current_sequence_idx += 1

            # Predicted frame + predicted state (if predict_future_states)
            if cfg.predict_future_states:
                if cfg.use_third_person_image:
                    image_sequence.append(primary_image_duplicated.copy())
                    future_image_latent_idx = current_sequence_idx
                    current_sequence_idx += 1
            ...
            # No value prediction for retrieval layout
            value_latent_idx = -1
            ...
            # ── Non-retrieval layout (original order) ────────────────────────────
            # Proprio
            if cfg.use_proprio:
                image_sequence.append(blank_image_duplicated.copy())
                current_proprio_latent_idx = current_sequence_idx
            ...
            # Future states
            if cfg.predict_future_states:
                # Initialize all future_* indices to -1 up front so any modality
                # that is disabled (e.g. wrist images) still has a valid -1
                # value when the return dict is assembled below. Without this,
                # combos like real_robot (predict_future_states=True +
                # use_wrist_image=False) would hit UnboundLocalError.
                future_proprio_latent_idx = -1
                future_wrist_image_latent_idx = -1
                future_wrist_image2_latent_idx = -1
                future_image_latent_idx = -1
                ...
                future_image_latent_idx = -1
                future_image2_latent_idx = -1
            if cfg.predict_values:
                image_sequence.append(blank_image_duplicated.copy())
                value_latent_idx = current_sequence_idx
                current_sequence_idx += 1
            else: value_latent_idx = -1
        ...
        print(raw_image_sequence.shape)  # Debug print to check shape of raw image sequence
        data_batch = {
            ...
            "retrieved_video_start_latent_idx": torch.tensor(
                [retrieved_video_start_latent_idx] * batch_size, dtype=torch.int64
            ).cuda(),
            "retrieved_video_end_latent_idx": torch.tensor(
                [retrieved_video_end_latent_idx] * batch_size, dtype=torch.int64
            ).cuda(),
            "retrieved_action_latent_idx": torch.tensor(
                [retrieved_action_latent_idx] * batch_size, dtype=torch.int64
            ).cuda(),
            **({"retrieved_actions": retrieved_actions_tensor} if retrieved_actions_tensor is not None else {}),
            "retrieved_state_latent_idx": torch.tensor(
                [retrieved_state_latent_idx] * batch_size, dtype=torch.int64
            ).cuda(),
            **({"retrieved_proprio": retrieved_proprio_tensor} if retrieved_proprio_tensor is not None else {}),
            **({"has_ret_image": torch.tensor([obs.get("has_ret_image", 1)] * batch_size, dtype=torch.float32).cuda()}
               if "retrieved_frames" in obs else {}),
        }
        ...
        generated_latent_with_action, orig_clean_latent_frames = model.generate_samples_from_batch(
            ...
            shift=cfg.shift,  # Shift parameter for rectified flow scheduler (predict2.5); ignored for predict2 (EDM) models via **kwargs
        )  # (B, C'=16, T', H'=28, W'=28)

        # Extract the predicted action chunk from the generated sample
        _raw_action = getattr(model, '_generated_action', None)
        if _raw_action is not None:
            # ActionProj model: action was predicted directly in action space (no encoder-decoder round-trip)
            actions = (
                _raw_action
                .reshape(batch_size, cfg.chunk_size, ACTION_DIM)
                .to(torch.float32)
                .cpu()
                .numpy()
            )
            model._generated_action = None  # clear after use
        else:
            ...
            actions = (
                extract_action_chunk_from_latent_sequence(
                    ...
                    decoder=getattr(model, 'action_decoder', None),
                )
                .to(torch.float32)
                .cpu()
                .numpy()
        ...
        if generate_future_state_and_value_in_parallel:
            ...
            if cfg.suite == "libero":
            ...
            elif cfg.suite == "robotwin":
                # Same segment ordering as ALOHA; see RoboTwinDataset.__getitem__.
                INDICES_TO_REPLACE = [
                    0,
                    1,
                    5,
            ...
            elif cfg.suite == "robotwin_ret":
                # 3-camera RAG layout (see RoboTwinRetDataset.__getitem__):
                # 0 blank, 1-3 ret head/L/R, 4 ret proprio, 5 ret action,
                # 6-8 cur head/L/R, 9 cur proprio, 10 action,
                # 11-13 future head/L/R, 14 future proprio.
                # Replace placeholder (non-image) slots with original latents
                # so VAE decode of the image slots stays clean.
                INDICES_TO_REPLACE = [0, 4, 5, 9, 10, 14]
            elif cfg.suite == "pusht":
                INDICES_TO_REPLACE = [
                    0,
                    1,
                    3,
                ]  # 0: blank, 1: curr proprio, 2: curr primary img, 3: action, 4: future proprio, 5: future primary img, 6: value
                future_wrist_image_latent_idx = -1
                future_wrist_image2_latent_idx = -1
                future_image2_latent_idx = -1
            elif cfg.suite == "real_robot":
                INDICES_TO_REPLACE = [
                    ...
                    1,
                    3,
                ]  # 0: blank, 1: curr proprio, 2: curr primary img, 3: action, 4: future proprio, 5: future primary img
                future_wrist_image_latent_idx = -1
                future_wrist_image2_latent_idx = -1
                future_image2_latent_idx = -1
            elif cfg.suite == "pusht_ret":
                INDICES_TO_REPLACE = [
                    0,
                    3,
                    4,
                    6,
                ]  # 0: blank, 1-2: ret_frame, 3: ret_state, 4: ret_action, 5: cur_frame, 6: cur_state, 7: pred_action
                future_wrist_image_latent_idx = -1
                future_wrist_image2_latent_idx = -1
                future_image2_latent_idx = -1
            else:
