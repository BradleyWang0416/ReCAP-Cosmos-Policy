...
class PushTEnv(gym.Env):
    ...
    * TODO:
    ...
    def __init__(
        ...
        self.success_threshold = 0.85  # 85% coverage
    ...
    def _initialize_observation_space(self):
        if self.obs_type == "state":
        ...
        elif self.obs_type == "state_and_pixels":
            self.observation_space = spaces.Dict(
                {
                    "state": spaces.Box(
                        low=np.array([0, 0, 0, 0, 0]),
            ...
            raise ValueError(
                ...
                "pixels_agent_pos, state_and_pixels]."
            )
    ...
    def step(self, action):
        ...
        # reward += - 1.0 * np.sum(np.abs(action - self.agent.position))
        # terminated = is_success = np.all(np.abs(action - self.agent.position) < 2.0)
    ...
    def reset(self, seed=None, options=None):
        ...
        if options is not None and options.get("reset_to_state") is not None:
            ...
            state = np.array(
                [
                    self.np_random.integers(50, 400),
                    self.np_random.integers(50, 400),
                    self.np_random.integers(100, 350),
                    self.np_random.integers(100, 350),
                    self.np_random.uniform(-np.pi/4, 3*np.pi/4),
                ]
            )
    ...
    def get_obs(self):
        ...
        if self.obs_type == "pixels":
            ...
            block_angle = self.block.angle % (2 * np.pi)
            return {
                "state": np.concatenate([agent_position, block_position, [block_angle]], dtype=np.float64),
                "pixels": pixels,
            }
    ...
    def _setup(self, options=None):
        ...
        if type(options) == dict and options.get("agent_shape") == "triangle":
            self.agent = self.add_triangle(self.space, (256, 400), 15)
        else:
            self.agent = self.add_circle(self.space, (256, 400), 15)
        if type(options) == dict and options.get("block_color") is not None: # check of options has block_color key, else default to LightSlateGray
            block_color = options.get("block_color")
        else: block_color = "LightSlateGray"
        self.block, self._block_shapes = self.add_tee(self.space, (256, 300), 0, color=block_color)
        if type(options) == dict and options.get("goal_flipped"):
            self.goal_pose = np.array([256, 256, - np.pi / 4])
        elif type(options) == dict and options.get("goal_angle") is not None:
            angle = options.get("goal_angle")
            # to rad
            self.goal_pose = np.array([256, 256, np.deg2rad(angle)])
        else:          
    ...
    @staticmethod
    def add_triangle(space, position, size=20):
    ...
    @staticmethod
    def add_lshape(space, position, angle, scale=30, color="LightSlateGray", mask=None):
        if mask is None:
            mask = pymunk.ShapeFilter.ALL_MASKS()
        mass = 1
        length = 4
    ...
    @staticmethod
    def state_from_keypoints(keypoints: np.ndarray) -> np.ndarray:
        """Recover block (x, y, angle) from an (8, 2) keypoints array.

        Inverse of get_keypoints.  Uses the centroids of the two component
        rectangles so the result is robust to vertex ordering within each shape.

        Body-local centroids (scale=30):
          shape1 (horizontal bar): (0, 15)
          shape2 (vertical stem):  (0, 75)

        The vector from shape1-centroid to shape2-centroid in body space is
        (0, 60).  In world space that same vector equals R(angle) @ (0, 60),
        which yields the angle directly.

        Args:
            keypoints: (8, 2) array as returned by get_keypoints.
                       First 4 rows belong to shape1, last 4 to shape2.

        Returns:
            (3,) array [block_x, block_y, block_angle] matching the state
            format used by reset(options={"reset_to_state": ...}).
        """
        keypoints = np.asarray(keypoints, dtype=np.float64)
        c1 = keypoints[:4].mean(axis=0)   # centroid of shape1 in world space
        c2 = keypoints[4:].mean(axis=0)   # centroid of shape2 in world space

        # c2 - c1 = R(angle) @ (0, 60) = (-60 sin θ, 60 cos θ)
        diff = c2 - c1
        angle = np.arctan2(-diff[0], diff[1])

        # body.position = mean_world - R(angle) @ (0, 45)
        # where (0, 45) is the overall body-local centroid
        R = np.array([[np.cos(angle), -np.sin(angle)],
                      [np.sin(angle),  np.cos(angle)]])
        local_center = np.array([0.0, 45.0])
        body_pos = keypoints.mean(axis=0) - R @ local_center

        return np.array([body_pos[0], body_pos[1], angle % (2 * np.pi)])
