...
def _write_human2robot_v04_runtime_binding(config: Config, dataset, sampler) -> None:
    """Fail closed on the frozen v04 stage-5 runtime before optimizer step 1."""

    binding_path_value = os.environ.get("HUMAN2ROBOT_V04_STAGE5_RUNTIME_BINDING_PATH")
    required = (
        "HUMAN2ROBOT_V04_STAGE5_METHOD",
        "HUMAN2ROBOT_V04_STAGE5_TRAINING_PROTOCOL_SHA256",
        "HUMAN2ROBOT_V04_GPU_DEVICES",
    )
    missing = [name for name in required if not os.environ.get(name)]
    if not binding_path_value:
        missing.append("HUMAN2ROBOT_V04_STAGE5_RUNTIME_BINDING_PATH")
    if missing:
        raise RuntimeError(f"Missing formal Human2Robot v04 runtime bindings: {missing}")

    method = os.environ["HUMAN2ROBOT_V04_STAGE5_METHOD"]
    target = "residual" if method == "recap_hand_ret" else "absolute"
    actual = {
        "method": str(dataset.method_id),
        "target_representation": str(dataset.target_representation),
        "seed": int(config.trainer.seed),
        "max_optimizer_steps": int(config.trainer.max_iter),
        "batch_size_per_gpu": int(config.dataloader_train.batch_size),
        "gradient_accumulation_steps": int(config.trainer.grad_accum_iter),
        "effective_global_batch_size": int(parallel_state.get_data_parallel_world_size())
        * int(config.dataloader_train.batch_size)
        * int(config.trainer.grad_accum_iter),
        "world_size": int(torch.distributed.get_world_size()),
        "data_parallel_world_size": int(parallel_state.get_data_parallel_world_size()),
        "visible_cuda_device_count": int(torch.cuda.device_count()),
        "fsdp_shard_size": int(config.model.config.fsdp_shard_size),
        "save_every_steps": int(config.checkpoint.save_iter),
        "H_steps": int(dataset.h_steps),
        "K_steps": int(dataset.k_steps),
        "top_k": int(dataset.top_k),
        "pool_size": int(dataset.pool_size),
        "retrieval_modality": str(dataset.retrieval_modality),
        "resolution": int(dataset.final_image_size),
        "action_loss_multiplier": int(config.model.config.action_loss_multiplier),
        "text_conditioning": str(dataset.text_conditioning),
        "sampler_class": type(sampler).__name__,
        "sampler_seed": int(sampler.seed),
        "initialization_checkpoint": str(config.checkpoint.load_path),
        "tokenizer_checkpoint": str(config.model.config.tokenizer.vae_pth),
        "training_protocol_sha256": str(dataset.protocol_file_sha256),
    }
    expected = {
        "method": method,
        ...
        "effective_global_batch_size": 200,
        "world_size": 4,
        "data_parallel_world_size": 4,
        "visible_cuda_device_count": 4,
        "fsdp_shard_size": 4,
        ...
        "sampler_class": "TaskBalancedDistributedSampler",
        "sampler_seed": 20260711,
        "initialization_checkpoint": os.environ["COSMOS_PREDICT2P5_POSTTRAINED_CKPT"],
        "tokenizer_checkpoint": os.environ["COSMOS_PREDICT2P5_TOKENIZER_CKPT"],
        "training_protocol_sha256": os.environ["HUMAN2ROBOT_V04_STAGE5_TRAINING_PROTOCOL_SHA256"],
    }
    if actual != expected:
        raise RuntimeError(f"Formal Human2Robot v04 runtime mismatch: actual={actual}, expected={expected}")

    if torch.distributed.get_rank() == 0:
        payload = {
            "schema_version": "human2robot-v04-stage5-runtime-binding-v1",
            "status": "BOUND_BEFORE_OPTIMIZER_STEP_1",
            "recorded_at_utc": datetime.now(timezone.utc).isoformat(),
            "host_gpu_devices": os.environ["HUMAN2ROBOT_V04_GPU_DEVICES"],
            "container_logical_gpu_devices": [0, 1, 2, 3],
            "actual": actual,
            "data_contract": {
                "source_root": str(dataset.source_root),
                "training_index_path": str(dataset.training_index_path),
                "statistics_path": str(dataset.statistics_path),
                "stage4_protocol_lock_path": str(dataset.stage4_protocol_lock_path),
                "split": str(dataset.split),
                "task_count": len(dataset.task_record_indices),
                "episode_count": len(dataset.records),
                "legal_window_count": len(dataset.starts),
                "expanded_sample_count": len(dataset),
                "heldout_data_used": False,
            },
            "offline_environment": {
                "COSMOS_SKIP_HF_AUTO_DOWNLOAD": os.environ.get("COSMOS_SKIP_HF_AUTO_DOWNLOAD"),
...
def launch(config: Config, args: argparse.Namespace) -> None:
    ...
    with data_loader_init():
        ...
        if not hasattr(dataset, "make_distributed_sampler"):
            raise RuntimeError("Human2Robot v04 dataset does not expose its frozen task-balanced sampler")
        sampler = dataset.make_distributed_sampler(
            num_replicas=parallel_state.get_data_parallel_world_size(),
            rank=parallel_state.get_data_parallel_rank(),
        )
        if int(sampler.seed) != sampler_seed:
            raise RuntimeError(f"Human2Robot v04 sampler seed mismatch: {sampler.seed} != {sampler_seed}")
        _write_human2robot_v04_runtime_binding(config, dataset, sampler)
