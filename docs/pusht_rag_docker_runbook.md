...
launcher 绑定本地 `cosmos-policy:latest` 的 image ID，以读写方式挂载仓库、`/DATA1` 和两个缓存目录，设置 `--ipc=host` 与离线环境，并禁用容器网络。它只生成 Docker session/preflight 回执并打开 shell，不启动 prepare、train 或 evaluate。禁止宿主机 Python、依赖同步和现场下载。

`tools/human2robot_v04.py` 已被阶段 1 split manifest 绑定 SHA，只保留为阶段 0/1 冻结后端，不再修改。阶段 3 起，唯一公开实验入口为 `/workspace/.venv/bin/python tools/human2robot_v04_experiment.py ...`。该入口的八个固定子命令全部默认 dry-run；任何数据写入、特征生成、训练或评估都必须显式给出 `--execute`，且仍需通过对应阶段门禁。
...
```bash
# dry-run：只生成本次独立 manifest、日志和 immutable receipt
.venv/bin/python tools/human2robot_v04_experiment.py prepare-features
.venv/bin/python tools/human2robot_v04_experiment.py train --method no_retrieval
.venv/bin/python tools/human2robot_v04_experiment.py evaluate --split dev --method recap_hand_ret --pool-size 10

# 正式环境门禁；只有显式 --execute 才运行完整 preflight
.venv/bin/python tools/human2robot_v04_experiment.py preflight --execute
```

阶段 3 只验收接口、八状态 v04 状态机和审计闭环。`prepare-features`、`train`、`evaluate`、`evaluate-oracle-phase`、`report` 即使带 `--execute`，在对应阶段 4～7 尚未实现和授权时也必须返回 `BLOCKED_STAGE_GATE`，不得生成特征、启动训练或打开 final。

每个命令在 `/DATA1/wxs/ReCAP_M5B_V04_RUNS/orchestrator_logs/<run_id>/` 创建不可覆盖的 attempt 日志、命令、runtime、status、progress、独立 manifest 和 immutable receipt。`preflight --execute` 只有在镜像、挂载、Python 3.10、离线环境、四卡 CUDA/NCCL、编译扩展、数据、四个本地权重 SHA256、至少 300 GiB 可用空间、文档哈希和 v03 冻结验证全部通过时才返回 `PASSED`；否则返回退出码 2 和 `BLOCKED_ENVIRONMENT`，不得启动后续阶段。
