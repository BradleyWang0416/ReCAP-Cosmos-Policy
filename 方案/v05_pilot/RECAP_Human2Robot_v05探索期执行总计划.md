...
`tools/pilot/` 下现有的 16 个脚本全部属于类别 A，并已在其 README 中披露不满足 Docker-only 约束。E2-B5、E3、E4 属于类别 B。E1、E2.0、E2 的 B1–B3、E5 属于类别 A。
...
| E0.4 | 交付 `start_v05_pilot_docker.sh`（按张数自动选卡，见 §2.4） | 能以 `V05_PILOT_GPU_COUNT=N` 启动并在回执写出实际 GPU UUID | ⬜ 未开始（E3 的前置，**当前唯一挡住 E3/E4 的工程项**） |
...
| E0.4 pilot launcher | 0 | ⬜ | **当前唯一挡住 E3/E4/B5 的工程项** |
...
**关键路径（本次修订后）**：**E0.4 → E2-B5**，E1 与 E2.0 并行。

理由：论文的核心检索信号是物体状态（§3.4），而它一格都没测过。E2 的否决条件已收紧为"自研映射档与论文成分档同时失败"，因此 **B5 现在是决定整条路线生死的单点**，而它需要 GPU，进而需要 E0.4。E0.4 从"E3 的前置"上升为**最高优先级工程项**。
