...
**否决条件**：若 B1–B4 在 E1 选定的 K 下全部无法达到 `ρ ≤ 0.90`，判定 **Human2Robot 不适合作为 RECAP 人手→机器人实验的替代数据集**，停止本路线，转为 (a) 换数据集，或 (b) 把课题重新定位为同具身跨 episode 检索。
...
**关键路径**：**E1 → E2**。E2 是唯一可能否决整个路线的**否决点**，但它不是起点——其判据以 E1 选定的 K 为准，故 E1 是前置。

两者都是纯 CPU，不等 GPU 释放；E1 的消融框架改造后单轮约 5 秒，扩样成本很低，先做它几乎不占预算。

参考：已有消融中 B 通道的 ρ 在 K=8/32/64 上为 0.96 / 0.96 / 0.95，近乎不随 K 变化，故 B0 的否决结论不依赖 E1；但 B1–B3 是新表征，K 敏感性未知，不得假定同样平坦。

E3（B4）只需一张卡半小时，是 E2 中最可能过线的一档，可在 E3.0 权重问题解决后与 B1–B3 并行推进。四卡组留给 E4。
